...
class GameAPI:
    ...
    # === High-level Mission API Helpers ===
    def deploy_mcv(self, faction="己方"):
        """Deploy the MCV for the given faction."""
        params = {"targets": {"type": ["基地车"], "faction": faction}}
        resp = self._send_request("deploy", params)
        return self._handle_response(resp, "部署基地车失败")

    def place_building(self, building_name: str, location: Location):
        """Place a building at a given location."""
        params = {"building": building_name, "location": location.to_dict()}
        resp = self._send_request("place_building", params)
        return self._handle_response(resp, f"放置建筑 {building_name} 失败")

    def start_production(self, unit_name: str, quantity=1, auto_place=False):
        """Start production of a unit/building."""
        units = [{"unit_type": unit_name, "quantity": quantity}]
        params = {"units": units, "autoPlaceBuilding": auto_place}
        resp = self._send_request("start_production", params)
        return self._handle_response(resp, f"生产 {unit_name} 失败")

    def can_produce(self, unit_name: str):
        """Check if a unit/building can be produced."""
        resp = self._send_request("query_can_produce", {"unit_type": unit_name})
        return self._handle_response(resp, f"查询可生产性失败: {unit_name}")

    def wait_for_completion(self, wait_id: str, timeout=30):
        """Wait for a waitId to complete."""
        start = time.time()
        while time.time() - start < timeout:
            resp = self._send_request("query_wait_info", {"waitId": wait_id})
            data = self._handle_response(resp, "查询等待状态失败")
            if data.get("waitStatus") == "completed":
                return True
            if data.get("waitStatus") == "invalid":
                return False
            time.sleep(0.5)
        return False

    def find_build_location(self, offsets=None):
        """Find a build location near the screen center."""
        screen = self.screen_info_query()
        cx = (screen.ScreenMin.X + screen.ScreenMax.X) // 2
        cy = (screen.ScreenMin.Y + screen.ScreenMax.Y) // 2
        if not offsets:
            offsets = [(0,0),(3,0),(0,3),(-3,0),(0,-3),(4,2),(-4,2)]
        for ox, oy in offsets:
            return Location(cx + ox, cy + oy)
        return Location(cx, cy)

    def run_mission_90s(self, faction="己方", time_limit=90):
        """Run the 90-second mission as per requirements."""
        start_time = time.time()
        def time_ok(): return time.time() - start_time < time_limit

        # 1. Deploy MCV
        if not time_ok(): return
        self.deploy_mcv(faction=faction)

        # 2. PowerPlant
        if not time_ok(): return
        loc = self.find_build_location()
        self.place_building("PowerPlant", loc)

        # 3. OreRefinery
        if not time_ok(): return
        loc = self.find_build_location()
        self.place_building("OreRefinery", loc)

        # 4. Infantry x3 (if possible)
        if not time_ok(): return
        can_info = self.can_produce("步兵")
        if can_info.get("can_produce", True):
            self.start_production("步兵", 3)

        # 5. WarFactory
        if not time_ok(): return
        loc = self.find_build_location()
        self.place_building("WarFactory", loc)

        # 6. MobileAA x2
        if not time_ok(): return
        can_info = self.can_produce("防空车")
        if can_info.get("can_produce", True):
            self.start_production("防空车", 2)

        # 7. Radar
        if not time_ok(): return
        loc = self.find_build_location()
        self.place_building("Radar", loc)

        # 8. AdvancedPowerPlant
        if not time_ok(): return
        loc = self.find_build_location()
        self.place_building("AdvancedPowerPlant", loc)
